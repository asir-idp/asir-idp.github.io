<#
Windows 7 S-NODE installation x86 (32 bits)
version: 20211028
...
function Install-7zip() {
    If (Test-Path "$env:ProgramFiles\7-zip") {
        ...
        Start-Process ".\7z1900.msi" -ArgumentList "/passive" -Wait
    }
}
...
function Install-ngrok() {
    If (Test-Path "$env:ProgramFiles\ngrok\ngrok.exe") {
        ...
        Unzip-File "ngrok-stable-windows-386.zip" "$env:ProgramFiles\ngrok"
        [Environment]::SetEnvironmentVariable("Path", $env:Path + ";" + "$env:ProgramFiles\ngrok", "Machine")
    }
}

function Install-OpenSSH() {
    If (Test-Path "$env:ProgramFiles\OpenSSH-Win32") {
        Write-Host "OpenSSH-Win32 for Windows already installed"
    } else {
        $file = "OpenSSH-Win32.zip"
        Write-Host "Unzipping OpenSSH..."
        Unzip-File $file "$env:ProgramFiles\OpenSSH-Win32"
    }
}

Write-Host "[0/5.INFO] WINDOWS 7 S-NODE installation (32 bits)"
...
& "$env:ProgramFiles\OpenSSH-Win32\install-sshd.ps1" | Out-Null
